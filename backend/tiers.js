...
export async function linkTierWithPermission(req,rest){
    const client= getClient();
    const token = req.headers.authorization.split(" ")[1];
    const userId = jwt.decode(token);
...
export async function deletePermission(req, rest){
    const client = getClient();
    const token = req.headers.authorization.split(" ")[1];
    const userId = jwt.decode(token);
...
export async function changePermissionOnTiers(req, rest){
    const client = getClient();
    const token = req.headers.authorization.split(" ")[1];
    const userId = jwt.decode(token);
